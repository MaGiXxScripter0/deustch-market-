-- Pickup appointment, handover code and durable picking checklist.
alter table public.requests
  add column if not exists pickup_slot_start timestamptz,
  add column if not exists pickup_code text;

update public.requests
set pickup_code = upper(substring(md5(id::text) from 1 for 6))
where pickup_code is null;

alter table public.requests
  alter column pickup_code set not null;

create unique index if not exists requests_pickup_code_unique_idx
  on public.requests (pickup_code);

alter table public.request_items
  add column if not exists picked_qty numeric(12, 3) not null default 0
  check (picked_qty >= 0);

drop function if exists public.place_pickup_order(text, text, text, text, boolean, jsonb);

create function public.place_pickup_order(
...
begin
  ...
  -- Deterministic ordering prevents two overlapping carts from deadlocking on inventory rows.
  for v_item in
    select value
  ...
  loop
    ...
    where product_id = v_product.id
      and location_id = (select id from public.locations where slug = 'berlin-mitte');
...
revoke execute on function public.place_pickup_order(text, text, text, timestamptz, text, boolean, jsonb)
  from public, anon, authenticated;
grant execute on function public.place_pickup_order(text, text, text, timestamptz, text, boolean, jsonb)
...
revoke execute on function public.set_pickup_item_picked(uuid, boolean) from public, anon;
grant execute on function public.set_pickup_item_picked(uuid, boolean) to authenticated;
